...
The bottom panel has two tabs — Activity and Git — plus an optional third [Custom panel](/tmux-agent-sidebar/features/custom-panel/) tab when `@sidebar_panel_command` is configured. Swap with `Shift+Tab`, or click the tab label with the mouse.

## Hiding the tab

```tmux
set -g @sidebar_show_activity_tab off
```

The tab disappears from the tab bar, `Shift+Tab` cycles what's left, and panes that were sitting on Activity move to the next tab that exists. The agent-started auto-switch to Activity stops happening too, since there is nothing to switch to. `@sidebar_show_git_tab` does the same for the Git tab; turn off every tab and the bottom panel is not drawn at all, exactly as `@sidebar_bottom_height 0` does.

Both toggles are re-read whenever the sidebar's window regains focus, so a live `tmux set -g` takes effect without restarting the sidebar.
