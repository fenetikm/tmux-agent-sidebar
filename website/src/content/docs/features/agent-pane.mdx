...
**Line 1** combines the status icon, a provider glyph (`✳` Claude, `◆` Codex, `◇` OpenCode), an optional permission-mode badge, the git branch, and right-aligned elapsed time.

**Line 2** shows one contextual detail, chosen in this order:

1. Wait reason (when waiting or errored)
...
Compact mode hides the session name, listening ports, task progress, the subagent tree, the `×` worktree remove marker, and prompt continuation lines. Sidebar-spawned worktrees can still be removed with `x`. Override the provider glyphs via `@sidebar_icon_agent_*` if a glyph renders double-width in your terminal.
